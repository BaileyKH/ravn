...
export default function Nearby({ navigation }) {
  ...
  const fetchPlaces = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Permission Denied', 'We need location to show nearby food.');
      setLoading(false);
      setRefreshing(false);
      return;
    }
    ...
    try {
      ...
      Alert.alert('Error', 'Something went wrong while fetching food places.');
    }
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.loadingWrapper}>
        <ActivityIndicator size="large" color="#FF3B3F" style={{ marginBottom: 16 }} />
        <Text style={styles.loadingText}>Looking for food near you...</Text>
      </SafeAreaView>
    );
  }  
          ...
          ListHeaderComponent={() => (
            <View style={styles.headerContainer}>
              ...
              <View style={styles.headerWrapper}>
                <Text style={styles.headerTitle}>Spots Nearby</Text>
              ...
              >
                <Text style={styles.dropdownToggleText}>Filters</Text>
                <FontAwesome
                      ...
                      {[1, 2, 3, 4].map((level) => (
                        <TouchableOpacity
                          ...
                          style={[
                            ...
                            filters.maxPrice === level && styles.dropdownOptionActive,
                          ]}
                          onPress={() =>
                            setFilters((prev) => ({ ...prev, maxPrice: level }))
                          }
                        >
                          <Text style={styles.dropdownText}>{'$'.repeat(level)}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                  </View>
                    ...
                    <View style={styles.dropdownRow}>
                      {[0.5, 1, 2, 5, 10].map((miles) => (
                        <TouchableOpacity
                          ...
                          style={[
                            styles.dropdownOption,
                            filters.maxDistance === miles && styles.dropdownOptionActive,
                          ]}
                          onPress={() =>
                            setFilters((prev) => ({ ...prev, maxDistance: miles }))
                          }
                        >
                          <Text style={styles.dropdownText}>{miles}mi</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                  </View>
...
function AnimatedCard({ item, index, location, openInMaps, getDistanceInMiles }) {
    ...
    >
      ...
      >
        ...
        <View style={styles.infoRow}>
          ...
          {item.price_level && (
            <Text style={styles.detail}>{'💲'.repeat(item.price_level)}</Text>
          )}
        ...
        {distance && (
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8 }}>
            <FontAwesome
              ...
              style={{ marginRight: 6 }}
            />
            <Text style={styles.distanceTag}>{distance} miles away</Text>
          </View>
        )}
